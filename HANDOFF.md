...
### 4.4 構成・モデル比較（`bench_asr.py`、test_wavs 50.5 秒の CER）
...
同じニュース読み上げドメインで、現行モデルに有利）。読み上げ原稿を
`bench_data/prompts.txt` に用意してある。録音したら
`bench_data/transcript.txt` に `<wav名> <実際に読んだ全文>` を書いて
`bench_asr.py bench_data` で回す。
