...
7b. **セッション管理は均一な並行処理**（接続 = セッション、main は共有
   状態を持たない）。変遷を記録する: 切り忘れ対策として「最新接続が先行
   セッションを蹴る」preemption を一度実装したが、その調整機構（Weak 保持・
   shutdown・join）自体が追加の状態であり、実際に fd リーク由来の
   デッドロックを生んだ。「例外処理を足すのと同一処理の並行より前者が
   単純か」というユーザー指摘で撤去。性能面の懸念も誤りだった（decode は
   エンジン側で走るためクライアントの非力さは無関係。並走が 400ms 周期に
   触れるのは最悪時のみで、影響も部分表示の遅延に限る）
7c. [x] **無音タイムアウト**: 無音が続いたらエンジンがセッションを打ち切る
   （既定 120 秒、`--idle-timeout` 秒で変更、0 で無効）。切り忘れの回収は
   これに一本化。**接続を閉じるだけで STOPPED は送らない** — クライアントは
