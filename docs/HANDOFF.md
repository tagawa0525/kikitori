...
7. [~] **Rust クライアント**:
   - [x] CLI 版 `kikitori-cli`（client/ クレート）: マイク → デーモン →
     ターミナル逐次表示 → Ctrl+C で確定、--wtype で入力。**ユーザーの
     実音声テスト済み**（技術文書の読み上げで全経路動作を確認）
   - 実地テストの知見: 誤認識はカタカナ英語の技術語彙（sherpa→シェルパー、
     ALSA→アルザ、libstdc++→リベース等）と同音異義（自律→自立）に集中。
     一般語彙はよく取れている。→ **置換辞書（§2 の hyprwhspr 由来案）の
     優先度を上げる**。エンジン側で確定テキストに適用する設計が良い
     （全クライアントで共有できるため）
...
7b. **セッションは高々 1 本・最新接続優先**（実装済み。設計判断であり
   技術制約ではない）: OfflineRecognizer は Send+Sync で並行化自体は可能。
   単一話者の道具なので「2 本目のセッションは切り忘れかバグ」であり、
   最新優先にすると切り忘れがトグルの瞬間に解消する。これが唯一の実質的
   根拠（+ 状態の単純さ）。**性能は根拠にならない**（ユーザー指摘で訂正）:
   decode は常にエンジン側（r995, 16T）で走るためクライアントの非力さは
   無関係で、並走が 400ms 周期に触れるのは両セッションとも 25 秒バッファの
   最悪時のみ。周期割れの影響も部分表示が遅れるだけで、確定と精度には
   影響しない。複数人で 1 台のエンジンを共用する要件が出たらこの排他を外し、
   並行セッション + 無音タイムアウト回収に切り替える（プロトコル・
   クライアントは変更不要）。実装上の注意: 接続の強いクローンを main が
   持つとセッション終了後もソケットが閉じず、クライアントが EOF 待ちで
   永久ブロックする（実際に起きた）。Weak で保持すること
7c. [ ] **無音タイムアウト**（ユーザー要望・合意済み）: 無音が数分続いたら
   エンジンがセッションを打ち切る。is_speech で無音サンプル数を累積し、
   閾値超過で**接続を閉じるだけ**（STOPPED は送らない。クライアントは
   切断検知で入力せずに終了するため、放置後にフォーカス先へ誤入力する
   事故が構造的に起きない）
